<template>
  <div>
    ...
    <div class="garage" v-if="getOwnedGarages.length > 0">
      <div class="garage__title" v-if="getOwnedGarages.length > 1">
        Informacje o warsztatach
      </div>
      <div class="garage__title" v-else>Informacje o warsztacie</div>
      <table
        class="garage__table"
        v-for="(garage, id) in getOwnedGarages"
        :key="id"
      >
        <tr>
          <th>Nazwa serwisu:</th>
    ...
    <p class="garage__message" v-else>
      Nie posiadasz jeszcze swojego warsztatu!
    </p>
  </div>
</template>
<script>
